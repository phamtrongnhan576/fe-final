...
export default function ListPosition() {
  ...
  const positionsFilter = positions.filter((position: PositionWithSlug, index: number) => {
    if (index < 8) {
      return position;
    }
  });
  ...
  const translatePositions = listInforPositions.map((position) => ({
    ...position,
    duration: t(position.duration)
  }));

  return (
    <div className="container mx-auto px-4 py-12">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {positionsFilter.map((position: PositionWithSlug, index: number) => (
          <Link
            key={position.id}
              ...
              <CardContent className="p-5 flex items-center gap-4 w-full">
                <div className="relative flex-shrink-0">
                  <div className="relative w-14 h-14 rounded-lg  border border-gray-100 dark:border-gray-600">
                    <Image
                      src={translatePositions[index].image}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 text-sm flex items-center gap-1.5 mt-1">
                    ...
                    <span className="truncate">{translatePositions[index].duration}</span>
                  </p>
                </div>
              </CardContent>
            </Card>
