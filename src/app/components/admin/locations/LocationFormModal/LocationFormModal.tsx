...
const LocationFormModal: React.FC<LocationFormModalProps> = ({
  ...
  const uploadImageMutation = useMutation({
    mutationFn: async ({ maViTri, file }: { maViTri: number; file: File }) => {
      ...
      return http.post<Location>(`vi-tri/upload-hinh-vitri?maViTri=${maViTri}`, formData);
    },
    onSuccess: (data) => {
      if (data?.hinhAnh) {
    ...
    >
      <Form layout="vertical" onFinish={formik.handleSubmit}>
        <div className="grid grid-cols-1">
          ...
          <Form.Item
            label="Country"
            validateStatus={formik.touched.quocGia && formik.errors.quocGia ? "error" : ""}
                ...
                <div className="flex items-center gap-2 w-full">
                  <Upload
                    ...
                    beforeUpload={(file) => {
                      ...
                      uploadImageMutation.mutate({ maViTri: formik.values.id, file });
                      return false;
                    }}
          ...
          <div className="w-full h-auto mb-4">
            {formik.values.hinhAnh && !formik.errors.hinhAnh ? (
              <img
                ...
                alt="Preview image"
                className="rounded"
                style={{ height: "150px", maxHeight: "200px", width: "100%", objectFit: "cover" }}
                onError={(e) => {
              ...
              >
                <span className="text-gray-500">
                  {formik.errors.hinhAnh ? formik.errors.hinhAnh : "No image preview"}
                </span>
              </div>
            )}
          </div>
