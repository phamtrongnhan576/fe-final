...
import { Modal, Form, Input, InputNumber, TreeSelect, Upload, Button, message } from "antd";
...
const RoomFormModal: React.FC<RoomFormModalProps> = ({
  ...
  const formik = useFormik<Room & { tenViTri?: string; tinhThanh?: string; tienNghi?: string[] }>({
    initialValues: {
      id: currentRoom?.id || 0,
    ...
    validationSchema: Yup.object({
      ...
      giaTien: Yup.number().min(0, "Price cannot be negative").required("Required"),
      ...
      tinhThanh: Yup.string().when("maViTri", {
        ...
        then: (schema) => schema.required("City/Province could not be loaded or does not exist"),
        otherwise: (schema) => schema.optional(),
      }),
      tenViTri: Yup.string().when("maViTri", {
        is: (maViTri: number) => maViTri && maViTri > 0,
        then: (schema) => schema.required("Location could not be loaded or does not exist"),
        otherwise: (schema) => schema.optional(),
      }),
    }),
    onSubmit: (values) => {
  ...
  >({
    ...
    queryFn: async () => {
      ...
      const res = await http.get<Location | null>(`/vi-tri/${formik.values.maViTri}`);
      return res || null;
    },
  ...
  useEffect(() => {
    ...
    if (maViTriValue > 0 && isLocationLoading) {
      formik.setValues(
        (prev) => ({ ...prev, tenViTri: "Loading...", tinhThanh: "Loading..." }),
        false
      );
      formik.setErrors({ ...formik.errors, maViTri: undefined, tenViTri: undefined, tinhThanh: undefined });
      setIsImagePreviewable(false);
    } else if (locationData && maViTriValue > 0) {
      formik.setValues(
        (prev) => ({ ...prev, tenViTri: locationData.tenViTri, tinhThanh: locationData.tinhThanh }),
        false
      );
      formik.setErrors({ ...formik.errors, maViTri: undefined, tenViTri: undefined, tinhThanh: undefined });
      setIsImagePreviewable(true);
      if (currentRoom && currentRoom.maViTri === maViTriValue && formik.values.hinhAnh === currentRoom.hinhAnh) {
      ...
      formik.setErrors({
        ...
        tenViTri: "Location does not exist (due to invalid/unfound location ID)",
        tinhThanh: "City/Province does not exist (due to invalid/unfound location ID)",
      });
      setIsImagePreviewable(false);
    } else {
      setIsImagePreviewable(true);
    }
  }, [isOpen, locationData, isLocationLoading, isLocationError, formik.values.maViTri, formik.touched.maViTri, currentRoom]);
  ...
  const uploadImageMutation = useMutation({
    mutationFn: async ({ maPhong, file }: { maPhong: number; file: File }) => {
      ...
      return http.post<Room>(`/phong-thue/upload-hinh-phong?maPhong=${maPhong}`, formData);
    },
    onSuccess: (data: Room) => {
...
const uploadProps: UploadProps = {
  ...
  return (
    <Modal
      title={currentRoom?.id ? "Edit Room" : "Add Room"}
      open={isOpen}
    ...
    >
      <Form layout="vertical" onFinish={formik.handleSubmit}>
        <div className="grid grid-cols-2 gap-4">
          <Form.Item
            label="Room Name"
            validateStatus={formik.touched.tenPhong && formik.errors.tenPhong ? "error" : ""}
          ...
          <Form.Item
            label="Location ID"
            validateStatus={formik.touched.maViTri && formik.errors.maViTri ? "error" : ""}
        ...
        <div className="grid grid-cols-2 gap-4">
          <Form.Item
            label="Location Name"
            validateStatus={formik.touched.tenViTri && formik.errors.tenViTri ? "error" : ""}
          ...
          <Form.Item
            label="City/Province"
            validateStatus={formik.touched.tinhThanh && formik.errors.tinhThanh ? "error" : ""}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex justify-between gap-4">
            <Form.Item
              label="Guests"
              validateStatus={formik.touched.khach && formik.errors.khach ? "error" : ""}
            ...
            <Form.Item
              label="Bedrooms"
              validateStatus={formik.touched.phongNgu && formik.errors.phongNgu ? "error" : ""}
            ...
            >
              <InputNumber
                ...
                onChange={(value) => formik.setFieldValue("phongNgu", value || 1)}
                onBlur={formik.handleBlur}
                min={1}
                className="w-full"
              />
          ...
          <div className="flex justify-between gap-4">
            <Form.Item
              label="Beds"
              validateStatus={formik.touched.giuong && formik.errors.giuong ? "error" : ""}
            ...
            <Form.Item
              label="Bathrooms"
              validateStatus={formik.touched.phongTam && formik.errors.phongTam ? "error" : ""}
            ...
            >
              <InputNumber
                ...
                onChange={(value) => formik.setFieldValue("phongTam", value || 1)}
                onBlur={formik.handleBlur}
                min={1}
                className="w-full"
              />
        ...
        <div className="grid grid-cols-2 gap-4">
          <Form.Item
            label="Amenities"
            validateStatus={formik.touched.tienNghi && formik.errors.tienNghi ? "error" : ""}
          ...
          <Form.Item
            label="Price (VND)"
            validateStatus={formik.touched.giaTien && formik.errors.giaTien ? "error" : ""}
...
<div className="grid grid-cols-2 gap-4 items-center">
      ...
      <Form.Item label="Upload Image">
        ...
        {uploadImageMutation.isPending && <p>Uploading image...</p>}
         ...
         <Form.Item
          label="Image URL"
          validateStatus={formik.touched.hinhAnh && formik.errors.hinhAnh ? "error" : ""}
        ...
        <div className="w-full h-auto mb-4">
          {isImagePreviewable && formik.values.hinhAnh && !formik.errors.hinhAnh ? (
            <img
              ...
              onError={(e) => {
                if (isImagePreviewable) {
                  formik.setFieldError("hinhAnh", "Unable to load image from this URL");
                }
              }}
            />
          ) : (
        ...
        <Form.Item
          label="Description"
          validateStatus={formik.touched.moTa && formik.errors.moTa ? "error" : ""}
